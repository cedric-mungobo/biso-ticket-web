<template>
    <OrganizerNavigation>
        <div class="p-1">
            ...
            >
                <div
                    class="mx-auto flex items-center justify-center h-12 w-12 rounded-md bg-gray-100 mb-4"
                >
                    <svg
                ...
                <UButton
                    ...
                    color="primary"
                    variant="solid"
                    >Créer votre premier événement</UButton
                >
            </div>
            ...
            <div v-else>
                <div
                    class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-2 gap-6"
                >
                    <UCard
                    ...
                    >
                        <div class="flex items-start gap-3">
                            <NuxtImg
                                @click="
                                    router.push(
                                        `/organisateur/events/${ev.id}?mode=view`,
                                    )
                                "
                                loading="lazy"
                                placeholder
                                ...
                                <div class="flex items-center gap-1.5 pt-2">
                                    ...
                                    <UTooltip text="Supprimer l'événement">
                                        <UButton
                                            ...
                                            @click="
                                                handleDeleteEvent(Number(ev.id))
                                            "
        ...
        >
            ...
            <div
                v-if="getEventToDelete"
                class="mb-4 p-3 bg-red-50 border border-red-200 rounded-md"
            >
                <p class="text-sm font-medium text-red-800">
                    Événement à supprimer :
                    <span class="font-semibold">{{
                        getEventToDelete.name
                    }}</span>
                </p>
                <p class="text-xs text-red-600 mt-1">
                    {{ formatDate(getEventToDelete.date_time) }} •
                    {{ getEventToDelete.location }}
                </p>
            </div>

            <p class="text-gray-700 mb-4">
                Êtes-vous sûr de vouloir supprimer cet événement ? Cette action
                est irréversible et supprimera :
            </p>
...
const { fetchMyEvents, formatDate, events, loading, error, deleteEvent } =
    useOrganizerEvents();
...
const confirmDeleteEvent = async () => {
    ...
    try {
        ...
        if (error?.message?.includes("404"))
            ...
            errorMessage = "Impossible de supprimer (réservations existantes)";
...
const copyEventLink = async (eventId: number) => {
    ...
    try {
        ...
        useAppToast().showSuccess(
            "Lien copié",
            "Le lien de l'événement a été copié dans le presse-papiers",
        );
