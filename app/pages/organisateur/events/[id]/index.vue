        ...
        <div class="p-2 md:p-0">
            <div class="">
                ...
                <div class="">
                    ...
                    <!-- Cartes d'action responsive -->
                    <div
                        class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3"
                    >
                        <!-- Cartes d'action -->
                        <template v-for="card in actionCards" :key="card.id">
                            <div
                                v-if="!card.condition || card.condition()"
                        ...
                        <div
                            v-if="
                                event?.settings?.scanSecret || event?.publicId
                            "
                            class="col-span-2 lg:col-span-1 space-y-2 sm:space-y-0 sm:flex sm:items-center sm:gap-4"
                        >
                            <!-- Clé de scan -->
                            <div
                                v-if="event?.settings?.scanSecret"
                                class="flex items-center gap-2 w-full"
                            >
                                <span
                                    class="text-xs sm:text-sm text-gray-700 whitespace-nowrap font-medium"
                                    >Clé de scan:</span
                                >
                ...
                <div
                    v-if="error"
                    class="p-4 rounded-md bg-red-50 border border-red-200 text-red-800"
                >
                    <div class="flex items-center">
                        ...
                        />
                        <span
                            >Impossible de charger l'événement. Veuillez
                            réessayer.</span
                        >
                    </div>
                </div>
                ...
                <div
                    v-else-if="event"
                    class="space-y-6 py-2 border border-gray-200 p-2 rounded-xl"
                >
                    <div class="overflow-hidden transition-all duration-300">
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-0">
                            <!-- Image à gauche - plus large -->
                            <div
                                class="relative lg:col-span-2 min-h-64 lg:h-full"
                            >
                                <img
                                    v-if="event?.image_url || event?.image"
                                    :src="event?.image_url || event?.image"
                                    :alt="event?.name"
                                    class="w-full h-full min-h-64 rounded-xl lg:h-full object-cover"
                                />
                                <div
                                    v-else
                                ...
                                <!-- Overlay avec statut sur l'image -->
                                <div
                                ...
                                >
                                    <UBadge
                                        ...
                                        variant="solid"
                                        size="lg"
                                        class="capitalize shadow-lg backdrop-blur-sm bg-opacity-90"
                                    >
                                        {{ event?.status }}
                                    </UBadge>
                                    <UBadge
                                        ...
                                        variant="solid"
                                        size="lg"
                                        class="shadow-lg backdrop-blur-sm bg-opacity-90"
                                    >
                                        <UIcon
                            ...
                            <!-- Informations à droite - design moderne -->
                            <div
                                class="p-6 lg:p-8 space-y-6 bg-gradient-to-b from-white to-gray-50"
                            >
                                <!-- En-tête avec titre -->
                                <div class="space-y-3">
                                    <h2
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="flex items-start space-x-3">
                                        <div
                                            class="flex items-center justify-center w-10 h-10 rounded-lg bg-primary-100 flex-shrink-0"
                                        >
                                            <UIcon
                                    ...
                                    <div class="flex items-start space-x-3">
                                        <div
                                            class="flex items-center justify-center w-10 h-10 rounded-lg bg-secondary-100 flex-shrink-0"
                                        >
                                            <UIcon
                                                name="i-heroicons-map-pin"
                                    ...
                                    >
                                        <div
                                            class="flex items-center justify-center w-10 h-10 rounded-lg bg-success-100 flex-shrink-0"
                                        >
                                            <UIcon
                                ...
                                <div class="pt-4 border-t border-gray-200">
                                    ...
                                    <p
                                        class="text-sm text-gray-600 leading-relaxed line-clamp-4 hover:line-clamp-none transition-all duration-300"
                                    >
                                        {{
                                            event?.description ||
                                ...
                                <div class="pt-4 flex gap-2">
                                    <UButton
                                        ...
                                        @click="
                                            navigateTo(
                                                `/organisateur/events/${eventId}/edit`,
                                            )
                                        "
                                    >
                                        <UIcon
                                    ...
                                    <UButton
                                        ...
                                        @click="
                                            navigateTo(
                                                `/organisateur/events/${eventId}/invitations`,
                                            )
                                        "
                                    >
                                        <UIcon
                ...
                <div class="modal-content-mobile">
                    ...
                    <div v-else class="space-y-3">
                        ...
                        >
                            ...
                            <div
                                class="flex flex-col sm:flex-row sm:items-center gap-2 mb-3 sm:mb-0"
                            >
                                <span
                                    class="text-sm font-medium text-gray-900"
                                    >{{ t.type }}</span
                                ...
                                <div class="flex items-center gap-2">
                                    <UBadge variant="soft" class="text-xs"
                                        >{{ t.quantity }} dispo</UBadge
                                    >
                                    <span
            ...
            >
                <div class="modal-content-mobile">
                    <TicketForm
                        ...
                        @cancel="showTicketCreate = false"
                        @submit="handleCreateTicket"
                    />
                </div>
            </Modal>
            <Modal
                v-model="showTicketEdit"
            ...
            >
                ...
                <template #footer>
                    <UButton variant="ghost" @click="showTicketDelete = false"
                        >Annuler</UButton
                    >
                    <UButton
                        color="error"
            ...
            <Modal v-model="showSponsorDelete" title="Supprimer le sponsor">
                <div class="modal-content-mobile">
                    <p class="text-sm text-gray-600">
                        Confirmer la suppression du sponsor "{{
                            currentSponsor?.name
                        }}" ?
                    </p>
                </div>
                <template #footer>
                    <UButton variant="ghost" @click="showSponsorDelete = false"
                        >Annuler</UButton
                    >
                    <UButton
                        color="error"
                        :loading="sponsorsSubmitting"
                        @click="handleDeleteSponsor"
                        >Supprimer</UButton
                    >
                </template>
            </Modal>
...
const actionCards = computed(() => [
    ...
    },
    {
        ...
        title: "Formulaires de réservation",
    ...
    },
    {
        id: "sponsors",
        title: "Mes sponsors",
        description: "Gérer les sponsors",
...
const { pending: ticketsPending, refresh: refreshTickets } = await useAsyncData(
    `organizer-event-tickets-${eventId}`,
    async () => {
...
const { pending: sponsorsPending, refresh: refreshSponsors } =
    await useAsyncData(`organizer-sponsors-${eventId}`, async () => {
...
const copyScanSecret = async () => {
    try {
        ...
        useToast().add({
            title: "Copié",
            description: "Clé de scan copiée.",
            color: "success",
        });
    } catch {
...
const openCreateReservationForm = () => {
    ...
    useAppToast().showInfo(
        "Fonctionnalité à venir",
        "La création de formulaires de réservation sera bientôt disponible.",
    );
};
...
const openViewReservationForm = (form: any) => {
    ...
    useAppToast().showInfo(
        "Fonctionnalité à venir",
        "La visualisation des formulaires de réservation sera bientôt disponible.",
    );
};
...
const openEditReservationForm = (form: any) => {
    ...
    useAppToast().showInfo(
        "Fonctionnalité à venir",
        "L'édition des formulaires de réservation sera bientôt disponible.",
    );
};
...
const openDeleteReservationForm = (form: any) => {
    ...
    useAppToast().showInfo(
        "Fonctionnalité à venir",
        "La suppression des formulaires de réservation sera bientôt disponible.",
    );
};
...
const handleCreateSponsor = async (payload: any) => {
    try {
        ...
        useAppToast().showSuccess(
            "Sponsor ajouté",
            "Le sponsor a été créé avec succès.",
        );
    } catch (e: any) {
...
const handleUpdateSponsor = async (payload: any) => {
    ...
    try {
        ...
        useAppToast().showSuccess(
            "Sponsor mis à jour",
            "Le sponsor a été modifié avec succès.",
        );
    } catch (e: any) {
        useAppToast().showError(
...
const handleDeleteSponsor = async () => {
    ...
    try {
        ...
        useAppToast().showSuccess(
            "Sponsor supprimé",
            "Le sponsor a été supprimé avec succès.",
        );
    } catch (e: any) {
        useAppToast().showError(
