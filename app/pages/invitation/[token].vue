<template>
    <div class="relative min-h-screen overflow-hidden">
        ...
        <div
            v-else-if="!isLoading && !hasError && !currentTemplate"
            class="min-h-screen flex items-center justify-center bg-gray-50"
        >
            <LoadingOverlay
                :show="true"
...
const loadTemplateComponent = async (designKey: string) => {
    ...
    try {
        ...
        const component = await import(
            `~/components/invitation/${designKey}.vue`
        );
        console.log(`Composant ${designKey}.vue chargé avec succès`);
        return component.default || component;
    } catch (error) {
        console.warn(
            `Composant ${designKey} non trouvé, utilisation du template par défaut`,
        );
        // Fallback vers le template par défaut
        const fallback = await import(
            "~/components/invitation/template_default.vue"
        );
...
const load = async () => {
    ...
    try {
        ...
        // Tracker la première visite pour l'auto-confirmation
        const visitKey = `invitation_visited_${token.value}`;
        const hasVisitedBefore = localStorage.getItem(visitKey);

        if (!hasVisitedBefore) {
            // Première visite - marquer comme visitée et stocker
            localStorage.setItem(visitKey, "true");
            invitation.value._isFirstVisit = true;
        }

        // Récupérer le design_key du template d'invitation depuis les settings de l'événement
        const designKey =
            event.value?.settings?.defaultInvitationTemplate?.designKey ||
