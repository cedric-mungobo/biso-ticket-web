...
export const useGuestBook = ({ slug, eventId, token }: UseGuestBookOptions) => {
  ...
  const canSubmitMessage = computed<boolean>(
    () =>
      guestBookContent.value.trim().length > 0 &&
      guestBookContent.value.length <= 2000,
  );
  ...
  const load = async (): Promise<void> => {
    ...
    try {
      ...
      invitationError.value =
        e?.message || "Erreur lors du chargement de l'invitation";
  ...
  const confirm = async (): Promise<void> => {
    ...
    try {
      ...
      useToast().add({
        title: "Erreur",
        description: String(
          e?.message || "Impossible de confirmer votre présence.",
        ),
        color: "error",
      });
    } finally {
  ...
  const loadGuestMessages = async (): Promise<void> => {
    if (!slug.value) {
      console.warn(
        "Slug d'événement non disponible, chargement des messages désactivé",
      );
      guestMessages.value = [];
      return;
    }
    ...
    try {
      const res = await $myFetch<any>(
        `/public/events/${slug.value}/messages?per_page=15`,
        {
          // Marquer cette erreur 404 comme déjà gérée pour éviter le toast global
          _handledError: true,
        },
      );
      const data = res?.data || res;
      ...
      // Si l'événement n'existe pas (404), initialiser avec un tableau vide silencieusement
      if (e?.status === 404) {
        console.log(
          "ℹ️ Événement non trouvé, initialisation avec tableau vide (attendu pour certains événements)",
        );
        guestMessages.value = [];
        guestMessagesError.value = "";
      } else {
        ...
        guestMessages.value = [];
        guestMessagesError.value =
          e?.message || "Erreur lors du chargement des messages";
      }
    } finally {
  ...
  const submitGuestBookMessage = async (): Promise<void> => {
    ...
    try {
      ...
      useAppToast().showError(
        "Erreur",
        String(e?.message || "Impossible d'envoyer."),
      );
    } finally {
