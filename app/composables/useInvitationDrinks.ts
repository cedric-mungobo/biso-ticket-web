...
export const useInvitationDrinks = () => {
  ...
  const fetchInvitationDrinkChoices = async (
    invitationId: number,
  ): Promise<DrinkChoice[]> => {
    try {
      ...
      if (process.dev)
        console.error(
          "Erreur lors de la récupération des choix de boissons:",
          error,
        );
  ...
  const saveInvitationDrinkChoices = async (
    invitationId: number,
    choices: DrinkChoice[],
    hasSaltFreeDiet?: boolean,
  ): Promise<DrinkChoice[]> => {
    try {
      const response = await $myFetch<DrinkChoiceResponse>(
        `/public/invitations/${invitationId}/drink-choice`,
        {
          method: "PUT",
          body: {
            drink_choice: choices,
            has_salt_free_diet: hasSaltFreeDiet || false,
          },
        },
      );
      return response.data || [];
      ...
      if (process.dev)
        console.error(
          "Erreur lors de la sauvegarde des choix de boissons:",
          error,
        );
  ...
  const validateDrinkChoices = (choices: DrinkChoice[]): string[] => {
    ...
    if (names.length !== uniqueNames.size) {
      errors.push(
        "Vous ne pouvez pas sélectionner la même boisson plusieurs fois",
      );
    }
