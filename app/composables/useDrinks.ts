...
export const useDrinks = () => {
  ...
  const fetchEventDrinks = async (eventId: number): Promise<Drink[]> => {
    try {
      const response = await $myFetch<DrinksResponse>(
        `/public/events/${eventId}/drinks`,
      );
