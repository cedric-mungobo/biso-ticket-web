...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  const customFetch = $fetch.create({
    ...
    onResponse({ request, response, options }) {
      if (process.dev) {
        try {
          ...
          console.log(
            "[API]",
            meta.method || options.method || "GET",
            String(request),
            "→",
            response.status,
    ...
    onResponseError({ request, response, options }) {
      // Vérifier si cette erreur est déjà gérée par le composant (via header ou option)
      const isHandledError =
        options.headers.get("X-Handled-Error") === "true" ||
        (options as any)?._handledError === true;

      // Si c'est une erreur 404 gérée, ne logger que si en mode dev
      if (response.status === 404 && isHandledError && !process.dev) {
        return; // Silencieux en production pour les erreurs 404 gérées
      }
      ...
      switch (response.status) {
        ...
        case 403:
          ...
          errorMessage =
            "Non autorisé. Vous n'avez pas les permissions nécessaires.";
          break;

        case 404:
          // Ressource non trouvée - ignorer si gérée manuellement
          if (process.dev) console.error("Erreur 404: Ressource non trouvée");
          errorMessage =
            "Ressource non trouvée. L'élément demandé n'existe plus.";
          break;
      ...
      // ET si l'erreur n'est pas déjà gérée par le composant
      if (response.status !== 422 && !isHandledError) {
        try {
