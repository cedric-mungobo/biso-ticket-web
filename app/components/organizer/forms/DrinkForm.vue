    ...
    <div class="space-y-4">
        ...
        <div v-if="drinks.length > 0" class="space-y-2">
            <h3 class="text-sm font-medium text-gray-900">
                Boissons configurées ({{ drinks.length }})
            </h3>
...
const addDrink = async () => {
    ...
    const exists = drinks.value.some(
        (d) => d.name.toLowerCase() === newDrink.value.name.toLowerCase(),
    );
